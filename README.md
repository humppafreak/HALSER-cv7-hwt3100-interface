...
4. Access the web UI at `http://wind-hdg.local`
...
| Auto (2 turns) | Enters calibration mode and watches the heading readings itself, accumulating total rotation regardless of direction, and automatically exits once two full turns (720°) have been covered — no need to press Stop. Times out and exits on its own after 4 minutes if the sensor is never actually rotated, so it can't get stuck. |
...
**Dedicated FreeRTOS tasks for blocking I/O:** Both the CV7 (`NMEA0183IOTask`) and the HWT3100 (`Hwt3100HeadingReader`) run their serial I/O on their own FreeRTOS task rather than the ReactESP event loop, since each read cycle blocks waiting on the UART. `ValueProducer::emit()` is called directly from those tasks into objects the main loop task also reads (e.g. `RepeatExpiring` inside the N2K senders), with no mutex or queue between them. This is an accepted, known risk rather than a safe pattern — ESP32-C3 being single-core does *not* eliminate it, since FreeRTOS still preemptively interleaves task execution on that one core. See [Known Limitations](#known-limitations) below.
...
**Unsynchronized cross-task writes into shared state.** `NMEA0183IOTask` (for the CV7) and `Hwt3100HeadingReader` (for the HWT3100) each call `ValueProducer::emit()` directly from their own dedicated FreeRTOS task, straight into objects the main loop task also reads — most notably `RepeatExpiring`/`ExpiringValue` inside the N2K senders, whose `.get()` is called every 100ms from the main task's `onRepeat` callback. There is no mutex, critical section, or queue between the writer and the reader.

This is a genuine, if narrow, data race: SensESP's `ExpiringValue::update()` writes its value and its last-updated timestamp as two separate, unsynchronized statements. If the reader runs in the gap between those two writes — entirely possible under FreeRTOS's preemptive scheduler, even though the ESP32-C3 has only one physical core — it can observe a fresh value paired with a stale timestamp, or vice versa. In practice the observable effect is minor and self-correcting: at worst, an occasional spurious `N2kDoubleNA` on the next 100ms cycle, not data corruption or a crash.

This is a deliberate, accepted tradeoff rather than an oversight: fixing it properly would mean routing every cross-task value through a queue (as SensESP's own `TaskQueueProducer` is designed for) or wrapping every read/write in a critical section, both of which add real complexity for a race whose worst case is a one-cycle glitch. If this ever needs tightening (e.g. before deploying somewhere the occasional glitch genuinely matters), see [issue #5](https://github.com/humppafreak/HALSER-cv7-hwt3100-interface/issues/5) for the options considered.
